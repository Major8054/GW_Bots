...
Func Initialize($aGW, $bChangeTitle = True, $aUseStringLog = False, $aUseEventSystem = True)
	...
	If IsString($aGW) Then
		...
		For $i = 1 To $lProcessList[0][0]
			...
			$mGWProcHandle = 0
		Next
		Out("Found process ID: " & $mGWProcessId)
	Else
		...
		MemoryOpen($aGW)
		ScanForCharname()
		Out("Using provided process ID: " & $mGWProcessId)
	EndIf

	; Check if the process handle is valid
	If $mGWProcHandle = 0 Then
		Out("Invalid process handle")
		Return 0
	EndIf

	; Scan for memory addresses
	Scan()
	Out("Scanned for memory addresses")
	...
	If @error Then
		Out("Failed to read base pointer")
		Return 0
	EndIf
	SetValue('BasePointer', '0x' & Hex($mBasePointer, 8))
	Out("Read base pointer: 0x" & Hex($mBasePointer, 8))
...
;~ 	$lTemp = GetScannedAddress('ScanEngine', -0x6D + 2)    ;-16
	...
	If @error Then
		Out("Failed to read target log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read skill log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read skill complete log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read skill cancel log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read chat log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read trader hook address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read dialog log address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read string filter 1 address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read string filter 2 address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read trade hack address")
		Return 0
	EndIf
	...
	If @error Then
		Out("Failed to read queue counter")
		Return 0
	EndIf
	...
	If $bChangeTitle Then
		...
		If @error Then
			Out("Failed to change window title")
			Return 0
		EndIf
		Out("Window title changed")
	EndIf
...
Func BuySalvageKit2($aQuantity = 1)
...
;~ 							Out("Picking up "&GetItemName($lItem)&" from the ground.")
...
Func GoToNearestNPC($aX, $aY)
	...
	If Not IsDllStruct($lNearestAgent) Then
		Out("No agent found near the specified coordinates." & @CRLF)
		Return
	EndIf
...
Func GetNumberOfFoesInRangeOfAgent22($aAgent = -2, $aRange = 1250)
	Local $lAgent, $lDistance
	Local $lCount = 0
...
;Func Out($msg) ;Prints message in the Status console
