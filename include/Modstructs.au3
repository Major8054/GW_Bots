 Func IsPerfectShield($item) ; Need to add -5(20%)
	...
	; Universal mods
    Local $Plus30 = StringInStr($ModStruct, "001E4823", 0, 1) ; Mod struct for +30HP any weapon
 ...
 Func IsPerfectCaster($item)
	...
	Local $Restoration20Casting = StringInStr($ModStruct, "22141822", 0, 1)
...
Func IsPerfectMartial($item)
	...
	Local $15wEnch = StringInStr($ModStruct, "0F006822", 0, 1) ; 15wEnch
	Local $15above50 = StringInStr($ModStruct, "0F327822", 0, 1) ; 15^50
	Local $15wStance = StringInStr($ModStruct, "0F00A822", 0, 1) ; 15wStance
	Local $15wHex = StringInStr($ModStruct, "0F005822", 0, 1) ; 15wHex
	Local $15minus = StringInStr($ModStruct, "0F003822", 0, 1) ; 15 with negative condition (-5e, -1 regen,...)
	Local $minus1Health = StringInStr($ModStruct, "0100E820", 0, 1) ; -1 Health Regen

	if IsReq7Max($item) or IsReq8Max($item) or IsReq9Max($item) then
		if ($type = $itemType_axe or $type = $itemType_sword or $type = $itemType_bow) then
			if ($Oldschool5e > 0) then
				return true
			endif
		elseif $type = $itemType_daggers then
			if  ($Oldschool5e > 0) or ($15wEnch > 0) or ($15above50 > 0) or ($15wStance > 0) then
				return true
			endif
		endif
	elseif $Type = $itemType_sword or $Type = $itemType_axe then
		if ($15minus > 0 and $minus1Health > 0) then
			return true
		endif
	else
		return false
...
;	Local $Sentinel = StringInStr($ModStruct, "FB010824", 0, 1)
	...
	ElseIf($Vitae > 0) or ($Attunement > 0) or ($Clarity > 0) Then ; Neutral Runes
		if $Type = 2 or $Type = 5 or $Type = 15 or $Type = 26 or $Type = 27 or $Type = 32 or $Type = 35 or $Type = 36 then
			return False ; Clears false positives: +2e on weapon same modstruct as attunement
		Else
			Return True
		endif
...
Func IsReq7Max($item)
	...
	Switch $Rarity
    Case 2626, 2623, 2624 ; Blue, Purple, Gold
       Switch $Type
			Case 2 ; Axe
				If $Dmg == 25 And $Req == 7 Then return true
			Case 5 ; Bow
				If $Dmg == 25 And $Req == 7 Then return true
			Case 12 ; Focus
				If $Dmg == 11 And $Req == 7 Then return true
			; Case 15 ; Hammer
			; Case 22 ; Wand
			Case 24 ; Shield
				If $Dmg == 15 And $Req == 7 Then return true
			; Case 26 ; Staff
			Case 27 ; Sword
				If $Dmg == 20 And $Req == 7 Then return true
			Case 32 ; Daggers
				If $Dmg == 15 And $Req == 7 Then return true
			; Case 35 ; Scythe
			; Case 36 ; Spear
...
Func IsReq8Max($item)
    ...
    Case 2626, 2623, 2624 ; Blue, Purple, Gold
       Switch $Type
			Case 2 ; Axe
				If $Dmg == 28 And $Req == 8 Then return true
			Case 5 ; Bow
				If ($Dmg == 27 or $Dmg == 28) And $Req == 8 Then return true
			Case 12 ; Focus
				If $Dmg == 12 And $Req == 8 Then return true
			; Case 15 ; Hammer
			; Case 22 ; Wand
			Case 24 ; Shield
				If $Dmg == 16 And $Req == 8 Then return true
			; Case 26 ; Staff
			Case 27 ; Sword
				If $Dmg == 22 And $Req == 8 Then return true
			Case 32 ; Daggers
				If ($Dmg == 16 or $Dmg == 17) And $Req == 8 Then return true
			; Case 35 ; Scythe
			; Case 36 ; Spear
		endSwitch
		Return False
...
Func IsReq9Max($item)
    ...
    Case 2626, 2623, 2624 ; Blue, Purple, Gold
       Switch $Type
			Case 2 ; Axe
				If $Dmg == 28 And $Req == 9 Then return true
			Case 5 ; Bow
				If $Dmg == 28 And $Req == 9 Then return true
			Case 12 ; Focus
				If $Dmg == 12 And $Req == 9 Then return true
			; Case 15 ; Hammer
			; Case 22 ; Wand
			Case 24 ; Shield
				If $Dmg == 16 And $Req == 9 Then return true
			; Case 26 ; Staff
			Case 27 ; Sword
				If $Dmg == 22 And $Req == 9 Then return true
			Case 32 ; Daggers
				If $Dmg == 17 And $Req == 9 Then return true
			; Case 35 ; Scythe
			; Case 36 ; Spear
		endSwitch
		return False
