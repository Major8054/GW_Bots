...
Global Const $ITEM_ID_TOMES = 21797; mesmer tomes
...
Global $gwpid = -1
Global $useScroll = False; USESCROLL
Global $picktomes = False
Global $pickpcons = True
Global $pickevent = False
...
Global Const $EventItemModelID2 = $ITEM_ID_GLACIAL_STONES
Global Const $EventItemName2 = "gStones:" ; just for the interface.
Global Const $EventItemModelID3 = $ITEM_ID_LOCKPICKS
Global Const $EventItemName3 = "Lockpicks:" ; just for the interface.
...
Global $EventItemCount2 = 0
Global $EventItemCount3 = 0
...
; ==== GUI ====
#Region GUI
Global Const $mainGui = GUICreate("Vaettir Bot", 436, 320, 308, 185)
...
GUICtrlCreateLabel($EventItemName2, 8, 165, 70, 17)
Global Const $EventItemLabel2 = GUICtrlCreateLabel($EventItemCount2, 80, 165, 50, 17)

GUICtrlCreateLabel($EventItemName3, 8, 180, 70, 17)
Global Const $EventItemLabel3 = GUICtrlCreateLabel($EventItemCount3, 80, 180, 50, 17)
...
Global $LabelDeaths
GUICtrlCreateGroup("", -99, -99, 1, 1)
GUICtrlCreateGroup("OTHERS", 8, 260, 417, 57, BitOR($GUI_SS_DEFAULT_GROUP,$BS_CENTER))
Global $CheckboxScrolls = GUICtrlCreateCheckbox("Use Scroll", 12, 280, 100, 20) ; USESCROLL
    GUICtrlSetState(-1, $GUI_UNCHECKED)
	GUICtrlSetOnEvent(-1, "GuiOptionsHandler")
Global $CheckboxTomes = GUICtrlCreateCheckbox("Pick Tomes", 140, 280, 100, 20) ; USESCROLL
    GUICtrlSetState(-1, $GUI_UNCHECKED)
	GUICtrlSetOnEvent(-1, "GuiOptionsHandler")
Global $CheckboxPcons = GUICtrlCreateCheckbox("Pcons", 240, 280, 100, 20) ; USESCROLL
    GUICtrlSetState(-1, $GUI_CHECKED)
	GUICtrlSetOnEvent(-1, "GuiOptionsHandler")
Global $CheckboxEvent = GUICtrlCreateCheckbox("Event items", 340, 280, 100, 20)
    GUICtrlSetState(-1, $GUI_UNCHECKED)
    GUICtrlSetOnEvent(-1, "GuiOptionsHandler")
#EndRegion
...
Func GuiButtonHandler()
     If GUICtrlGetState($CheckboxScrolls) = $GUI_CHECKED Then; USESCROLL
		$useScroll = True ; use scrolls to get more skill points
     Elseif GUICtrlGetState($CheckboxScrolls) = $GUI_UNCHECKED Then
	    $useScroll = False
	 EndIf
     If GUICtrlGetState($CheckboxTomes) = $GUI_CHECKED Then; USESCROLL
		$picktomes = True
     Elseif GUICtrlGetState($CheckboxTomes) = $GUI_UNCHECKED Then
	    $picktomes = False
	 EndIf
	 If GUICtrlGetState($CheckboxPcons) = $GUI_CHECKED Then
		$pickpcons = True
     Elseif GUICtrlGetState($CheckboxPcons) = $GUI_UNCHECKED Then
	    $pickpcons = False
	 EndIf
	 If GUICtrlGetState($CheckboxEvent) = $GUI_CHECKED Then
		$pickevent = True
     Elseif GUICtrlGetState($CheckboxEvent) = $GUI_UNCHECKED Then
	    $pickevent = False
	 EndIf
	 Out("$useScroll = " & $useScroll)
	 Out("$picktomes = " & $picktomes)
	 Out("$pickpcons = " & $pickpcons)
	 Out("$pickevent = " & $pickevent)
	...
	EndIf
EndFunc
Out ("/!\ The bot doesn't pick up golds, if you want the bot pick up golds, change line 796 in this script /!\")
...
WEnd
; use a scroll exp before kill mobs
Func UseScroll()
...
Func IsRecharged2($lSkill); maj 2019
...
Func PickUpLoot()
	...
	For $i = 1 To GetMaxAgents()
		...
		If CanPickup($lItem) Then
			...
			EndIf
		    If DllStructGetData($lItem, "ModelID")==$EventItemModelID2 Then
				$EventItemCount2+=1
				GUICtrlSetData($EventItemLabel2, $EventItemCount2)
			EndIf
			If DllStructGetData($lItem, "ModelID")==$EventItemModelID3 Then
				$EventItemCount3+=1
				GUICtrlSetData($EventItemLabel3, $EventItemCount3)
			EndIf
		EndIf
	Next
EndFunc   ;==>PickUpLoot
...
Func CanPickUp($aItem)
	...
	If $pickevent = True Then
	   ...
	   If $lModelID == $ITEM_ID_Eggnog         Then Return True
	   If $lModelID == $ITEM_ID_Spiked_Eggnog  Then Return True
	   If $lModelID == $ITEM_ID_Frosty_Tonic   Then Return True
	   If $lModelID == $ITEM_ID_Snowman_Summoner  Then Return True
	   If $lModelID == $ITEM_ID_Fruitcake      Then Return True
	   If $lModelID ==	$ITEM_ID_Mischievious_Tonic  Then Return True
	   If $lModelID == $ITEM_ID_MARK           Then Return True
    EndIf
    ;==========tomes============
    If $lModelID == $ITEM_ID_TOMES and $picktomes = True Then return True
