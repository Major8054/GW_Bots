...
#include "include/Dependency.au3"
#include "include/Stockage.au3"
#include "include/Modstructs.au3"
...
Global Const $Salvage = True
Global Const $KeepSpiritwood = True
...
#Region Configuration
	Local Const $version = " v.2021-10-02"
...
#Region GUI
	...
	_GUICtrlComboBox_SetCurSel($gui_txtname,0)
	...
	Local Const $gui_edit = GUICtrlCreateEdit("===Dragon Moss Ranger===", 142, 5, 162, 165, BitOR(0x0040, 0x00200000, 0x00800000, 0x0800))
...
Out2("Salvage : " & $Salvage)
Out2("Keep Spiritwood : " & $KeepSpiritwood)
Out2("Build : OgcTcZ885RgNB1ZHQWZoT48cAA")
Out2("======================")
Out("Ready to start")
	...
	While 1
		If $running Then
			...
			If Not $resignReady Then
			   ...
			   DepositGold()
			   Out("Load skillbar")
			   LoadSkilltemplate("OgcTcZ885RgNB1ZHQWZoT48cAA") ; test retiré temp
			   $resignReady = True
			EndIf
			If DoJob() Then
			    ;out("dojob()")
...
Func DoJob()
	...
	_UseSkillex(3); SF
    If IsRecharged(3) Then ; s'il y a le bonus de moral
	  _UseSkillex(3); SF
	EndIf
	Out("Killing.")
	Sleep(250)
...
#Region Inventory
	...
	Func ManageInventory()
		...
		;If True Then
			...
			Out("Travel To Maatu")
			UseSpeedBoost()
			$Xunlai = GetNearestAgentToCoords(-11545,13089)
			Out("Open Xunlai Chest")
			GoToNPC($Xunlai)
			Out("Go To Merchant")
	...
	Func _inventory()
		...
		;_sell()
		SellDMLoot()
		If $Salvage Then _salvage()
		_store()
		Out("Storage")
		If Not IsChestFull() Then Storagef()
        DepositGold()
	    If (IsChestFull()) or(GetGoldCharacter() > 99000) Then
		   _TravelGH()
		   Out("Chest full ! Travel to Gh")
		   EctoBuy()
		   GUICtrlSetData($gui_btstart, "Resume")
		   $running = False
		   $initialized = False
		...
		Switch DllStructGetData($aItem, "Type")
			...
			Case 18
			 ...
			 Case 11
			    ...
			    If $lmodelid = 937 Then Return False ; rubis
			    If $lmodelid = 948 Then Return False ; fer
			    If $lmodelid = 929 Then Return False ; dust
			    If $lmodelid = 22280 Then Return False ; parcho fow
