...
;*Removed crashcheck.
;*Changed PickUpItems() to pickuploot().
;*Fixed Disable/Enable Rendering.
;*Added suport to multi accounts.
;*Added log function.
;Orignal script and credits: http://www.gamerevision.com/showthread.php?1833-GWA%B2-Kilroy-Stonekin-s-Punch-Out-Extravaganza!
;All changes are based on scripts from forum like Vaettir farm bot (from Ddarek) and PvEvolved (from TheArkanaProject)
...
$cGUI = GUICreate("GWA² - Kilroy Stonekin", 270, 342, 200, 180)
...
$txtName = GUICtrlCreateCombo($strName, 100, 32, 162, 20);, BitOR($ES_CENTER, $ES_AUTOHSCROLL))
...
Global $gLogBox = GUICtrlCreateEdit("", 8, 148, 260, 200, 2097220)
Global $fLog = FileOpen("Kilroy.log", 1) ;Log file
...
Func Ident($bagIndex)
	...
	For $i = 0 To DllStructGetData($bag, 'slots')-1
		out("Identifying item: " & $bagIndex & ", " & $i)
		If FindIDKit() = 0 Then
