...
Global $strName = "Missy Shiny"

; Build Template: OACiIykM5N1JWOdN5Di50M5MBA
...
Global $merch
Global $gwpid = -1
Global $Input = GUICtrlCreateCombo("", 8, 8, 137, 25)


#Region ### START Koda GUI section ### Form=C:\Program Files\AutoIt3\Koda Form Designer\Forms\ReFeather.kxf
$Form1 = GUICreate("ReFeather 1.0.5", 870, 290, 100, 100)
Global $charactername = GUICtrlCreateCombo("", 344, 24, 121, 21)
	GUICtrlSetData(-1, GetLoggedCharNames())

$startbutton = GUICtrlCreateButton("Start", 8, 224, 233, 57)
Global $disablegraphics = GUICtrlCreateCheckbox("Disable Graphics", 264, 80, 177, 17)
	GUICtrlSetOnEvent(-1, "ToggleRendering")
$storegolds = GUICtrlCreateCheckbox("Store golds at chest", 264, 104, 137, 17)
$storemats = GUICtrlCreateCheckbox("Store materials at chest",630, 260,137, 17)
$Group4 = GUICtrlCreateGroup("ID and sell from bag:", 496, 8, 113, 121)
GUICtrlCreateGroup("", -99, -99, 1, 1)
$usebag1 = GUICtrlCreateCheckbox("Nr. 1", 505, 32, 95, 17)
$usebag2 = GUICtrlCreateCheckbox("Nr. 2", 505, 56, 95, 17)
$usebag3 = GUICtrlCreateCheckbox("Nr. 3", 505, 80, 95, 17)
$usebag4 = GUICtrlCreateCheckbox("Nr. 4", 505, 104, 95, 17)
...
Global $Console = GUICtrlCreateEdit("Ready to Start", 620, 8, 245, 185, BitOR(0x0040, 0x00200000, 0x00800000, 0x0800))
...
GUICtrlSetOnEvent($disablegraphics,"ToggleRendering")
...
ReadConfig()
Out("Template build = OACiIykM5N1JWOdN5Di50M5MBA")
Out("Alternative build = OACjAeiIJPQzH318bWOPbNTnJA")
...
Func ReadConfig()
	if FileExists("config.ini") Then
		SetState($disablegraphics,IniRead("config.ini","general","disablegraphics","False"))
...
Func UpdateConfig()
	Local $gdata = "disablegraphics="&GetChecked($disablegraphics)&@LF&"storegolds="&GetChecked($storegolds)
	IniWriteSection("config.ini", "general", $gdata)
...
Func Init()
	...
	If $boolRun Then
		...
		Else
			If Initialize(GUICtrlRead($charactername), True, True) = False Then ; If Initialize(GUICtrlRead($charactername), True, True) = False Then
...
Func SellAndBack()
	...
	If InventoryCheck() Then                                      ;maby additional checkbox therefor
		$boolRun = False
		Out("Bot stopped, low inventory space")
		GUICtrlSetData($startbutton, "Start")
		GUICtrlSetState($charactername, $GUI_ENABLE)
		GUICtrlSetState($startbutton, $GUI_ENABLE)
...
Func HardLeave()
	...
	ReturnToOutpost()
	;Travel($LL); tesy car resign marche pas
...
Func Nuke()
	...
	Out("$target = GetNearestEnemyToAgent(-2)")
	Local $lMe = GetAgentByID(-2)
	Out("Local $lMe = GetAgentByID(-2)")
	Local $e = 0, $shouldmove = false
	Out("Local $e = 0, $shouldmove = false")
	Do
	   Out("boucle Do")
		if GetIsDead(-2) Then
			;HardLeave()
...
Func IDAndSell()
	RndSleep(1000)
	Out ("Store Golds - materials")
	MoveTo(17624,8904,100)
	Out("MoveTo(17624,8904,100)")
	Sleep(Random(150,500))
	GoNearestNPCToCoords(17624,8904)
	Out("GoNearestNPCToCoords(17624,8904)")
	If GetChecked($storegolds) Then
		Out("Looking for gold items")
		...
		EndIf
	 EndIf
	; If GetChecked($storemats) Then
	;	Out("Looking for materials")
	 ;   If GUICtrlRead($usebag1) == $GUI_Checked Then
		;	MatIs(1, 20)
	;	EndIf
	;	If GUICtrlRead($usebag2) == $GUI_Checked Then
	...
	Out("GoNearestNPCToCoords(17219,12378)")
	If GUICtrlRead($usebag1) == $GUI_Checked Then
	    Out("Ident(1)")
		Ident(1)
	EndIf
	...
	EndIf
	$intGold += GetGoldCharacter() - $intCash
	Out("$intGold += GetGoldCharacter() - $intCash")
	$intCash = GetGoldCharacter()
	GUICtrlSetData($gold, $intGold)
EndFunc   ;==>IDAndSell
...
Func GoldIs($bagIndex, $numOfSlots)
 ...
 Func MatIs($bagIndex, $numOfSlots)
	For $i = 1 To $numOfSlots
		ConsoleWrite("Checking items: " & $bagIndex & ", " & $i & @CRLF)
		$aItem = GetItemBySlot($bagIndex, $i)
		If (DllStructGetData($aItem, 'ID') = 8253) or (DllStructGetData($aItem, 'ID') = 7665)  Then
			Do
				For $bag = 8 To 11 ; 6 To 16 are all storage bags
					$slot = FindEmptySlot($bag)
					If $slot <> 0 Then
...
Func Ident($bagIndex)
	$bag = GetBag($bagIndex)
	Local $r = 0
	Out("DllStructGetData($bag, 'slots')" & DllStructGetData($bag, 'slots'))
...
Func CanSell($aitem)
	...
	ElseIf $m = 7665 Then ;Feathers
		Return False
	ElseIf $m = 22190 Then ;shamrock ales
		Return False
	ElseIf $m = 22191 Then ;clovers
		Return False
    ElseIf $m = 840 Then ;supply
		Return False
...
Func CanPickUp($aitem)
	...
	ElseIf $m = 933 Then ;Feathers    7665
		...
		Return True
	ElseIf $m = 22191 Or $m = 22190 Then ;event items clover/shamrock.  22644, 22752 = bunny/egg
		Return True
    ElseIf $m = 923 Or $m = 931 Or $m = 18345 Then ;claw/eyes/tokens
		Return True
    ElseIf $m = 22644 Or $m = 931 Or $m = 22752 Then ;claw/eyes/tokens
		Return True
    ElseIf $m = 840 Then ;supply
		Return True
...
Func GoNearestNPCToCoords($x, $y)
	Do
		...
		;Out("DllStructGetData($guy, 'Id')" &DllStructGetData($guy, 'Id'))
