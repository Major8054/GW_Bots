...
global const $champion = 1947;1943
Global Const $fow = 34
...
Func GuiButtonHandler()
	...
	Else
		...
		$BotInitialized = True
		 setmaxmemory()
		 LoadSkilltemplate("OgcTcZ88Z6ASn5uU4ABimsBKuEA")
	  EndIf
...
func enterkneel()
   ...
   elseif getmapid() = $toa Then
	  out("zoning to toa")
	  rndsleep(200)
	  if kneel(-2522,18731) Then WaitMapLoading($fow)
...
EndFunc
 func kneel($one,$two)
   ...
   local $FailPops = 0
   $spawned = False
  if not $spawned Then moveto($one,$two)
  ...
  If DllStructGetData($Avatar, "PlayerNumber") <> $champion Then
    if $ensuresafety Then
	 if not isdisempty() Then
	   ...
	   EndIf
	  EndIf
	  if getmapid() <> $town then Return false
		Out("kneeling")
		SendChat("kneel", "/")
		Local $lDeadlock = TimerInit()
		Sleep(1500)
		Do
		   if getmapid() <> $town then Return
			Sleep(1500)	; 1500.
			$Avatar = GetNearestNPCToCoords($one,$two)
			;$a = DllStructGetData($Avatar, "PlayerNumber")
			;Out("DllStructGetData($Avatar, PlayerNumber" & $a)
			;If DllStructGetData($Avatar, "PlayerNumber") == $champion Then
			;   GoNpc($Avatar)
			;   Out("entering fow")
			;   GoNpc($Avatar)
			...
			If TimerDiff($lDeadlock) > 5000 Then
				...
				$FailPops += 1
			EndIf
	    Until DllStructGetData($Avatar, "PlayerNumber") == $champion  or $FailPops = 2 or getmapid() == $fow;
	 EndIf
