...
Global Const $corridor = 284 ;if you want to start at corridor, change the follow line
global $town = $corridor ;start town
global $shardcount = 0
Global $wins = 0
Global $fails = 0
Global $shardcount = 0
...
$Button1 = GUICtrlCreateButton("Initalize", 8, 32, 107, 25)
...
Func UseSpeedBoost()
	$item = GetItemByModelID(21492)
	If DllStructGetData($item, 'Bag') <> 0 Then
		UseItem($item)
		Return
    EndIf
	$item = GetItemByModelID(22644)
	If DllStructGetData($item, 'Bag') <> 0 Then
		UseItem($item)
		Return
    EndIf
	$item = GetItemByModelID(21812)
	If DllStructGetData($item, 'Bag') <> 0 Then
		UseItem($item)
		Return
    EndIf
 ...
 func moveandcheckdis($ax,$ay,$arandom = 65) ; for noids , replace this with moveto() in kneel,will change dis as soon as it detect s1
	...
	Do
	    ...
	    if not isdisempty() Then
		   enterfow()		;; Starting Map is 284, ID: Zin Ku Corridor
...
Global Const $one = 8144
Global Const $two = -16655
 func kneel($one,$two)
	    UseSpeedBoost()
	  		;; Starting Map is 284, ID: Zin Ku Corridor
		;MoveTo(8144, -17658)
		;MoveTo(7557, -17626)
		MoveTo(7046, -17582)
		MoveTo(6551, -17435)
  ...
  If DllStructGetData($Avatar, "PlayerNumber") <> $champion Then
    if $ensuresafety Then
	 if not isdisempty() Then
	   ...
	   EndIf
	  EndIf
	   if getmapid() <> $corridor then Return
		Out("kneeling")
		SendChat("kneel", "/")
		Local $lDeadlock = TimerInit()
		Do
...
func isdisempty()
   ...
   if $peoplecount > 0 Then
	  ...
	  if $town = $chantry then
		 out("dis not empty, going Zin Ku Corridor")
	     $town = $corridor
	  Else
