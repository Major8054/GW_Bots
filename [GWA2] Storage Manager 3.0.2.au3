...
Global Enum $mFur = 941, $mLinen = 926,  $mDamask = 927,  $mSilk = 928,  $mEctoplasm = 930,  $mDeldrimorSteel = 950,  $mClaw = 923, $mEye = 931, $mFang = 932;, $mSteel = 949, TEST
...
Global $nXunlai[7] = ["Xunlai Chest", "Coffre Xunlai", "Xunlai-Truhe", "Forziere Xunlai", "Arc�n Xunlai", "Skrzynia Xunlai", "Xoonlaeee Chest"]
...
Global Const $MainGui = GUICreate("ITool v" & $version, 420, 300)
   ...
   Global $Console = GUICtrlCreateEdit("Ready to Start", 220, 8, 195, 290, BitOR(0x0040, 0x00200000, 0x00800000, 0x0800))
   ...
   Global Const $sSteel = GUICtrlCreateCheckbox("Steel", 130, 205)
   Global Const $sBoispirite = GUICtrlCreateCheckbox("Boispirite", 20, 225)
...
EndFunc ;==>_WM_LBUTTONDOWN
 ...
 Func Salvage()
   ...
   For $i = 1 To 4
	  If Not SalvageBag3($i) Then ExitLoop
...
;~ 				Sleep(GetPing() * 3 + 1000)
				...
				Local $bItem
				Do
				  If $boolRunning = False Then ExitLoop

				  Sleep(500 + GetPing());300
				  $bItem = GetItemBySlot($aBag, $i)
				  If (TimerDiff($lDeadlock) > 5000) Then ExitLoop ; 1000 (si crash, 10000)
				  Out("data = " & DllStructGetData($bItem, 'Quantity') & " = " &  $q - $j)


				  Out("qty : " & $q & " j :" & $j)
				;prendre ici
				;Until (DllStructGetData($bItem, 'Quantity') = ($q - 1))
			    Until (DllStructGetData($bItem, 'Quantity') = $q - $j)
			   ;$q = DllStructGetData($bItem, 'Quantity'); - 1
		...
		If $q >= 1 Then
		   ; quantity > 1
			...
			For $j = 1 To $q
				...
				SalvageKit()
				Out("$lSalvageCount " & $lSalvageCount)
...
;~ 				Sleep(GetPing() * 3 + 1000)
				...
				Do
				  ...
				  Sleep(500 + GetPing());300
				  $bItem = GetItemBySlot($aBag, $i)
				  If (TimerDiff($lDeadlock) > 10000) Then ExitLoop ; 1000 (si crash, 10000)
				  Out("data = " & DllStructGetData($bItem, 'Quantity') & " = " &  $q - $j)
...
Func SalvageBag($lBag)
	...
	Local $lSalvageType
	Local $lSalvageCount
	Out(DllStructGetData($aBag, 'Slots'))
	For $i = 1 To DllStructGetData($aBag, 'Slots')
		...
		Out("$lSalvageType = " & $lSalvageType)
		Switch $lSalvageType
			Case -1
				ContinueLoop
			Case 0, 1, 2
				StartSalvage($lItem)
				Out("Debug : Case 0-1-2")
				While (GetPing() > 1250)
					RndSleep(250)
					Out("Ping To High" & GetPing())
				WEnd
...
;~ 	If getchecked($mcLunarFortunes) Then _ArrayAdd($ArrayModelIDInv, $mLunarFortunes)
   ...
   If getchecked($sBoispirite) Then _ArrayAdd($ArraySalvageModelID, $mBoispirite)

   If getchecked($sSteel) Then _ArrayAdd($ArraySalvageModelID, $mSteel)
